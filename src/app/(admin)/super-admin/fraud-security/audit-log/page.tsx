  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <Stat
          label="Warnings"
          value={String(audits?.filter((a) => a.status === "Warning").length ?? 0)}
          color="text-amber-600"
        />
        <Stat
          label="Blocked"
          value={String(audits?.filter((a) => a.status === "Blocked").length ?? 0)}
          color="text-red-600"
        />
        <Stat
          label="System Actions"
          value={String(audits?.filter((a) => a.role === "SYSTEM_API").length ?? 0)}
          color="text-blue-600"
        />
      </div>
      ...
      <Card padded={false}>
        <div className="p-4 flex flex-col sm:flex-row items-center gap-3 border-b border-gray-100">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {["ALL", "SUPER_ADMIN", "FINANCIAL_ADMIN", "SUPPORT_ADMIN", "SYSTEM_API"].map(
              (r) => (
                <button
                  key={r}
